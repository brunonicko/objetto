...
class AttributeDescriptor(Slotted):
    ...
    def __init__(
        ...
        if type is not None and exact_type is not None:
            raise ValueError(
                "cannot specify bot 'type' and 'exact_type' arguments"
            )
        if type is not None:
...
class UpdateState(SlottedMutableMapping):
    ...
    def __init__(self, state, model):
        # type: (coll.Mapping[str, Any], Model) -> None
...
def _make_access_object(
    ...
    for name, members in iteritems(properties_functions):
        dct[name] = property(
            fget=members.get("fget"),
            fset=members.get("fset"),
...
def _make_model_class(mcs, name, bases, dct):
    # type: (Type[ModelMeta], str, Tuple[Type, ...], Dict[str, Any]) -> ModelMeta
    ...
    for base in reversed(cls.__mro__):
        for member_name, member in iteritems(base.__dict__):
            ...
            if isinstance(member, AttributeDescriptor) and isinstance(cls, ModelMeta):
...
class Model(with_metaclass(ModelMeta, EventListenerMixin, SlottedHashable, BaseModel)):
    """Object model described by attributes."""
